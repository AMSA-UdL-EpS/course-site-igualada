...
En aquest laboratori QEMU emula una màquina **x86 amb BIOS**. El nostre Stage 1 i Stage 2 s'executen en **mode real de 16 bits**: **no** estem fent un programa x86-64. Fem servir `qemu-system-i386` perquè GDB hi mostra els registres com `eip` i `esp` (de 32 bits), que contenen els `IP` i `SP` de 16 bits de la teoria.
:::
...
Instal·leu els paquets amb el gestor del vostre sistema (a Debian/Ubuntu, per exemple: `nasm`, `qemu-system-x86` i `gdb`) seria ```sudo apt-get update && sudo apt-get install nasm qemu-system-x86 gdb build-essential -y```. Si teniu processador basat en ARM, cal instal·lar ```sudo apt install gdb-multiarch -y```
...
:::{.errorcomu}
Si feu servir `-nographic` (vegeu més avall), no poseu `cli` abans del `hlt`: en les nostres proves, l'últim caràcter del missatge no arribava a la terminal. Per això el nostre bucle és `hlt` sense `cli`.
:::
...
```{.bash}
gdb -q
# Si teniu ARM, gdb-multiarch -q
```

Dins de GDB:

```bash
# Si teniu ARM, set architecture i386
source lab.gdb
target remote :1234
```

:::{.bonapractica}
GDB pot mostrar avisos sobre l'*executable* o l'*OS ABI*. Són normals: ignoreu-los.
:::
...
.load_stage2:
    ...
    ; TODO 1: DL = unitat d'arrencada (variable boot_drive) i AH = funció «lectura estesa»
    int  0x13
...
Terminal 1: `qemu-system-i386 -drive file=disk.img,format=raw -S -s`. Terminal 2: `gdb -q` i, dins de GDB:
...
## Activitat posterior — Trenca-ho

:::{.repte}
**Fora de la sessió.** Trenqueu l'arrencada de tres maneres i relacioneu cada símptoma amb la diapo de diagnòstic. Per cada cas: **prediu**, comproveu i digueu **en quina etapa ha fallat**: el firmware, Stage 1 o Stage 2. Treballeu sobre una **còpia** de `disk.img`.
:::
...
:::{.resum}
**Què hem après.** El BIOS només llegeix el sector 0 i el copia a `0x7C00`, on salta amb `DL` = unitat d'arrencada; tota la resta és feina de Stage 1. Amb GDB hem vist el codi a la RAM, els registres que **no** podem assumir i la diferència entre **carregar** i **executar**.

| Teoria | Laboratori |
|:--|:--|
| `0x7C00` (convenció del BIOS) | Part B: GDB s'atura a `0x7C00` |
| *boot sector* de 512 B, `55 AA` | Part A: `wc -c`, `hexdump -s 510` |
| mode real, `CS:IP` | Part B: `cs`, `eip` |
| «No assumim» / entorn mínim | Part B: `ds = es = ss = 0` per sort, `sp = 0x6f08` → `0x7c00` |
| `INT 10h` | `print` a Stage 1 i Stage 2 |
| `INT 13h AH = 0x42`, DAP | Part C |
| LBA ≠ adreça de memòria | `seek=2048` vs `0x8000` |
| carregar ≠ executar | Part C (sense el `jmp`) i Part D (GDB a `0x8000`) |
| *load address* ≠ *link address* | Ampliació: `be 14 80` vs `be 14 00` |
| diagnòstic | Activitat posterior |
:::

A la Part 2 de la unitat veurem què passa **després** de Stage 2: taules de particions, GRUB, el kernel i l'*initramfs*. L'activitat de casa continuarà amb UEFI.
